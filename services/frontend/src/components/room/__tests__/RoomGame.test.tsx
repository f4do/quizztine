...
describe("RoomGame", () => {
  ...
  it("shows source link when questionSourceUrl is set", () => {
    ...
    const link = screen.getByText((content) =>
      content.startsWith("room.source"),
    );
  ...
  it("does not show player counter in solo mode", () => {
    ...
    expect(screen.queryByText("room.players_answered")).not.toBeInTheDocument();
    expect(
      screen.queryByText("room.waiting_for_answers"),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders the question phase indicator", () => {
    ...
    expect(
      screen.getByText((content) => content.startsWith("room.question")),
    ).toBeInTheDocument();
  });
