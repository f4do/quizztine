...
describe("RoomPreGame", () => {
  ...
  it("shows nickname input for non-auth users when not joined", () => {
    ...
    expect(screen.getByPlaceholderText("room.nickname")).toBeInTheDocument();
  });
  ...
  it("shows disconnected indicator for disconnected players", () => {
    ...
    const bobBadge = screen.getByText("Bob").closest("span") as HTMLSpanElement;
  ...
  it("shows green dot for connected players", () => {
    ...
    expect(aliceBadge.querySelector(".bg-emerald-500")).toBeInTheDocument();
  });
