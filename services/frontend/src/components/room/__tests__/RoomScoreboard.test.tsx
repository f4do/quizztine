  ...
  return {
    scoreboard: [
      scoreboardEntry("p1", {
        nickname: "Alice",
        score: 120,
        cumulative_time: 15.3,
      }),
      scoreboardEntry("p2", {
        nickname: "Bob",
        score: 80,
        cumulative_time: 22.7,
      }),
      scoreboardEntry("p3", {
        nickname: "Charlie",
        score: 45,
        cumulative_time: 18.1,
      }),
    ],
    roomMode: "solo",
    creatorPid: null,
...
describe("RoomScoreboard", () => {
  ...
  it("hides play again button for non-creator in multiplayer", () => {
    ...
    expect(screen.queryByText("room.play_again")).not.toBeInTheDocument();
  });
  ...
  it("hides easter egg in solo mode even when all scores are 0", () => {
    render(
      <RoomScoreboard
        {...createProps({
          roomMode: "solo",
          scoreboard: [scoreboardEntry("p1", { nickname: "Alice", score: 0 })],
        })}
      />,
    );
    expect(screen.queryByText("room.easter_egg")).not.toBeInTheDocument();
  });
  ...
  it("handles single player scoreboard", () => {
    render(
      <RoomScoreboard
        {...createProps({
          scoreboard: [scoreboardEntry("p1", { nickname: "Alice", score: 50 })],
        })}
      />,
    );
    expect(screen.getByText("Alice")).toBeInTheDocument();
